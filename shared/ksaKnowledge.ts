...
TYPES: "markdown", "json", "csv", "text"`,
  ...
  beads: {
    system: `You have a pre-created task list in beads from the intent schema. Use beads.list() to see your tasks, update status as you work with beads.update(id, { status: 'in_progress' }), and close tasks with beads.close(id) when done.`,
    about: `WHEN TO USE: Tasks are already bootstrapped from intent schema goals - just update status as you work.

KEY PATTERN:
1. beads.list() - see pre-created tasks from goals
2. beads.update(id, { status: 'in_progress' }) - mark as started
3. beads.close(id) - mark as complete

DON'T create new beads unless breaking down a complex subtask into smaller pieces.
